...
function Action({ actionName, description, Icon }) {
    return (
        <div className="group bg-white shadow-lg p-6 sm:p-8 md:p-10 rounded-3xl 
            ...
            {/* Action name */}
            <h3 className="text-lg sm:text-xl font-semibold group-hover:text-white transition-colors duration-300 mt-5">
                {actionName}
            </h3>

            {/* Description */}
            <p className="text-sm sm:text-base text-[#1A1A18]/60 group-hover:text-white transition-colors duration-300 mt-2">
                {description}
            </p>
        </div>
    );
