...
function QuickGenerateOption() {
    ...
    const occasion = ["Formal", "Work", "Casual", "Social", "Travel", "Active", "Festive", "Home"];
    const[selectedOccasion, setSelectedOccasion] = useState(null);

   
    const handleGenerate = () =>{
        if(!selectedOccasion) return;
        navigate(`/generate_outfit?occasion=${selectedOccasion.toLowerCase()}`);
    }
                ...
                <ul className="gap-3 sm:gap-3  grid grid-cols-4 md:grid-cols-4 lg:grid-cols-8 ">
                    ...
                    {occasion.map((occasion) => (
                        <li key={occasion} >
                            ...
                            <Button type="button" className={`text-xs sm:text-sm md:text-base lg:text-base 
                            ${selectedOccasion === occasion
                                    ? 'border-[#D4AF37] border '
                                    : 'hover:border-[#D4AF37] border border-transparent'
                                }`} onClick={() => setSelectedOccasion(occasion)}><span className="px-0 lg:px-2 md:px-3 text-[#D4AF37] playfair-display ">{occasion}</span></Button>
                        </li>
                    ))}
                </ul>
            </div>
            <div className="flex md:hidden lg:hidden justify-end mt-3">
                <Button type="submit" variant="secondray" className="flex justify-center w-full " onClick={handleGenerate}><span className=" flex px-5 gap-3 work-sans font-bold"><WandSparkles />Generate Outfit</span></Button>
            </div>
        </div>
    )   
}
